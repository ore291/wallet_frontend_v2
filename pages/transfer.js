...
const transfer = () => {
  ...
  async function handleSubmit(e) {
    ...
    var rec_email = rec_emailRef.current.value;

    e.preventDefault();

    try {
      const res = await userService.transferBalance(amount, password, id, rec_email);
      router.push({
        pathname: "/",
      });
    } catch (error) {
      ...
      <form className="w-full max-w-sm" onSubmit={handleSubmit}>
        <div className="flex flex-col border-b border-teal-500 py-2">
          <div className="text-sm font-bold text-gray-700 tracking-wide">
            Enter Recepients Email
          </div>
          <input
            className="appearance-none bg-transparent border-none w-full text-gray-700 mr-3 py-1 px-2 leading-tight focus:outline-none"
            type="email"
            placeholder="xyz@knight.com"
            ref={rec_emailRef}
          />
        </div>
        <div className="flex flex-col border-b border-teal-500 py-2">
