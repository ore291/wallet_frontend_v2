...
const withdraw = () => {
  ...
  async function handleSubmit(e) {
    ...
    e.preventDefault();
      ...
      try {
          const res = await userService.withdrawBalance(amount, password, id );
          router.push({
            pathname: "/",
          });
      } catch (error) {
    ...
    <div className="flex justify-center p-24 bg-white mx-20 my-5 shadow-2xl rounded-lg">
      <form className="w-full max-w-sm" onSubmit={handleSubmit}>
        <div className="flex flex-col border-b border-teal-500 py-2">
            <div className="text-sm font-bold text-gray-700 tracking-wide">Enter bank Account</div>
          <input
        ...
        <div className="flex flex-col border-b border-teal-500 py-2">
            <div className="text-sm font-bold text-gray-700 tracking-wide">Enter amount to withdraw</div>
          <input
            className="appearance-none bg-transparent border-none w-full text-gray-700 mr-3 py-1 px-2 leading-tight focus:outline-none"
            type="number"
            placeholder="0.00"
